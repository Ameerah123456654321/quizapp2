...
async function  getData(){
    ...
    if(numbersQustions!=""&&difficulty!=null){
        ...
        let fetch1Json=await fetch1.json()
         ...
         toggelDiv(landing,oprations_div)
    ...
    if(index<data.length){
       ...
       let randomAnswers=random(answers);
        ...
        for(let i=0;i<randomAnswers.length;i++){
            ...
            div.className="div_check_answer";
...
function grade(result,total){
    let test=total/2
    if(result==total){
        result_span.innerHTML=`Excellent <i class="thum fa-solid fa-badge-check"></i>`
    }
    else if(result>test && result<total){
        result_span.innerHTML=`Good <i class="thum fa-solid fa-thumbs-up"></i>`;
    }else{
        result_span.innerHTML=`Unfortunately, try again <i class="thum fa-light fa-face-pensive"></i>`
    }
}
